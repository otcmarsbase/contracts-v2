...
describe('MAR-1125', () => {
  it('should send back remaining ETH in a static offer after bid', async () => {
    ...
    let txBid = await dex
      .connect(bob)
      .acceptOffer(id, usdt.address, '15558151500000000000');
  ...
  it('should close ETH offer after 3 bids', async () => {
    ...
    let { owner, alice, bob, dex, usdt, MarsBase, parseLogs } =
      await prepareEnvironment();
    ...
    let txBid1 = await dex
      .connect(bob)
      .acceptOffer(id, usdt.address, txParamsBob1.amountBob);
    let bidReceipt1 = await txBid1.wait();

    await usdt.connect(bob).approve(dex.address, txParamsBob2.amountBob);
    let txBid2 = await dex
      .connect(bob)
      .acceptOffer(id, usdt.address, txParamsBob2.amountBob);
    ...
    let amounts = [txParamsBob1, txParamsBob2, txParamsBob3].map((x) =>
      ethers.BigNumber.from(x.amountBob)
    );
    ...
    let txBid3 = dex
      .connect(bob)
      .acceptOffer(id, usdt.address, txParamsBob3.amountBob);
    await expect(txBid3).revertedWith('400-AAH');

    let amountBobWithoutOverflow = '12965126249999994000';
    let txBid4 = await dex
      .connect(bob)
      .acceptOffer(id, usdt.address, amountBobWithoutOverflow);
