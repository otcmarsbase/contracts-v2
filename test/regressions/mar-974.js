...
describe.skip('MAR-974', () => {
  it('should send tokens to the commission wallet if no exchange contract is specified', async () => {
    ...
    tx = await dex
      .connect(bob)
      .acceptOffer(id, usdt.address, '100000000000000000');

    receipt = await tx.wait();
    let acceptedEvent = receipt.events.find((x) => x.event == 'OfferAccepted');

    expect(acceptedEvent.args.amountAliceReceived).to.equal(
      '99500000000000000'
    );
    ...
    expect(await usdt.balanceOf(commission.address)).to.equal(
      '500000000000000'
    );
  });
  ...
  it("should not send tokens to the commission wallet if it's not specified", async () => {
    ...
    tx = await dex
      .connect(bob)
      .acceptOffer(id, usdt.address, '10000000000000000');
  ...
  it('should exchange tokens to USDT if the commission wallet and exchange address is specified', async () => {
    ...
    await expect(
      dex.connect(bob).acceptOffer(id, usdt.address, '100000000000000000')
