...
describe('features/auto close offers', () => {
        ...
        for (let SCS of ['0', '5000', '9999', '10000'])
          for (let DEADLINE of [0, Math.floor(Date.now() / 1000) + 86400])
            it(`should auto close ${
              ...
              DEADLINE ? '' : 'no '
            }deadline - min ${MIN_SIZE} minbid ${SCS}]`, async () => {
              ...
              let bidSuccess = await dex
                .connect(bob)
                .acceptOffer(0, bat.address, BID);
  ...
  it(
    'should convert offers with 100% smallest chunk size to 99.99% smallest chunk size'
  );
});
