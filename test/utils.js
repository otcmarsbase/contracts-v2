...
async function prepareJustContracts() {
  ...
  const DexAttackMock = await ethers.getContractFactory(
    'MarsBaseExchangeAttackMock'
  );
...
async function prepareEnvironment() {
  ...
  const { m, dex, bestBid, mplace, dexAttackMock } =
    await prepareJustContracts();
  ...
  return {
    ...
    mint: {
      ...
      tether: async (address, amount) => {
        await tether.issue(amount), await tether.transfer(address, amount);
      },
    },
    parseLogs: (logs) =>
