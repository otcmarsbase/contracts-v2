...
describe('missing/fee', () => {
  it('should correctly calculate fee for small amounts', async () => {
    ...
    for (let [amount, feeAmount] of [
      ...
      let [amountAfterFee, feeAmountResult] = await dex.afterFee(
        amount,
        feePercent
      );
      ...
      expect(
        feeAmountResult,
        `${amount} * ${feePercent / 10}% = ${feeAmountResult}`
      ).equal(feeAmount);
      expect(
        amountAfterFee,
        `${amount} - ${feePercent / 10}% = ${amountAfterFee}`
      ).equal(amount - feeAmountResult);
    }
  });
