...
describe('dynamic offers with minimum size', () => {
  it('should hold tokens before minimum size is covered before cancel', async () => {
    ...
    async function doOfferBids(offer) {
      for (let i = 0; i < 20; i++) {
        let bidSuccess = await dex
          .connect(bob)
          .acceptOffer(offer.id, bat.address, '1');
    ...
    let offer1 = await createOfferTokenToken(
      dex.connect(alice),
      usdt.address,
      '200',
      bat.address,
    ...
    await expectBalances({
      // offer created, bids not yet made
      alice: {
        usdt: '0',
        bat: '0',
      },
    ...
    await expectBalances({
      // tokens returned to respective owners
      alice: {
        usdt: '200',
        bat: '0',
      },
