...
describe("dynamic offers with deadlines", () =>
{
	...
	it("should not create an offer with deadline in the past", async () =>
	{
		...
		await expect(dex.connect(alice).createOffer(usdt.address, [bat.address], "100", ["200"], {
			...sensibleOfferDefaults(),
			deadline: deadline.toString(),
		})).revertedWith("405-OD")
	})
	it("should not bid in an expired offer", async () =>
	{
		...
		await expect(dex.connect(bob).acceptOffer(offer.id, bat.address, "25")).revertedWith("405-D")
	})
