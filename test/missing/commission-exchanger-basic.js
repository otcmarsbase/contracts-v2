...
describe('missing/commission-exchanger-basic', () => {
  ...
  it('should not set commission address as non-owner', async () => {
    ...
    await expect(
      dex.connect(alice).setCommissionAddress(alice.address)
    ).revertedWith('403');
    await expect(
      dex.connect(bob).setCommissionAddress(alice.address)
    ).revertedWith('403');
    await dex.setCommissionAddress(alice.address);
    await expect(dex.connect(alice).setCommissionAddress(ZERO)).revertedWith(
      '403'
    );
  });
  ...
  it('should not set commission exchanger as non-owner', async () => {
    ...
    await expect(
      dex.connect(alice).setExchangerAddress(alice.address)
    ).revertedWith('403');
    await expect(
      dex.connect(bob).setExchangerAddress(alice.address)
    ).revertedWith('403');
    await dex.setExchangerAddress(alice.address);
    await expect(dex.connect(alice).setExchangerAddress(ZERO)).revertedWith(
      '403'
    );
  });
});
