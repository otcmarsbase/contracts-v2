...
describe('missing/smallest chunk size', () => {
  for (let HOLD_TOKENS of [true, false])
    it(`should not allow bids less than the smallest chunk size for ${
      HOLD_TOKENS ? 'static' : 'dynamic'
    }`, async () => {
      ...
      await expect(
        dex.connect(bob).acceptOffer(offerId, bat.address, amounts.bidTooLow)
      ).revertedWith('400-AAL');

      // high bid should go okay
      let bid1 = await dex
        .connect(bob)
        .acceptOffer(offerId, bat.address, amounts.bidSuccess);
      await bid1.wait();
    ...
    for (let AMOUNT_ALICE of ['10000'])
      for (let SMALLEST_CHUNK_SIZE of ['5000'])
        for (let MIN_SIZE of [
          ...
          it(`[${
            HOLD_TOKENS ? 'static' : 'dynamic'
          } min size = ${MIN_SIZE}] should allow last bidder to bid less than smallest chunk size for all of the remaining tokens`, async () => {
            let env = await prepareEnvironment();
            ...
            // should fail
            await expect(
              dex.connect(bob).acceptOffer(offerId, bat.address, amounts.bid2),
              'too low - 1'
            ).revertedWith('400-AAL');
            ...
            let bid1 = await dex
              .connect(bob)
              .acceptOffer(offerId, bat.address, amounts.bid1);

            // too high bid should fail
            await expect(
              dex
                .connect(charlie)
                .acceptOffer(offerId, bat.address, amounts.bidTooHigh),
              'too high'
            ).revertedWith('');
            ...
            await expect(
              dex
                .connect(charlie)
                .acceptOffer(offerId, bat.address, amounts.bidTooLow),
              'too low - 2'
            ).revertedWith('400-AAL');
            ...
            let bid2 = await dex
              .connect(charlie)
              .acceptOffer(offerId, bat.address, amounts.bid2);
