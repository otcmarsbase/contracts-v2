...
describe("missing/eth", () =>
{
	...
	it("should create a bid with eth (hold tokens)", async () =>
	{
		...
		let offer = await createOfferTokenToken(dex.connect(alice), usdt.address, `4500${_000_}0`, ZERO, `1${_000_}0`, {
			holdTokens: true,
			feeAlice: 0,
			feeBob: 0,
			smallestChunkSize: 0,
		...
		for (let i = 0; i < 10; i++)
			await dex.connect(bob).acceptOffer(offer.id, ZERO, ethers.utils.parseEther("0.001"), {
		...
		// 4455 USDT expected
		let bid2 = await dex.connect(charlie).acceptOffer(offer.id, ZERO, ethers.utils.parseEther("0.990"), {
			value: ethers.utils.parseEther("0.990")
		})
		await expect(bid2).to.changeEtherBalance(alice, ethers.utils.parseEther("1.0"))
	...
	it("should not create a bid with eth if not enough eth", async () =>
	{
		...
		let tx = dex.connect(bob).acceptOffer(offer.id, ZERO, ethers.utils.parseEther("0.001"), {
			value: 0,
		})
