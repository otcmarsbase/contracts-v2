...
describe("missing/dynamic-offer", () =>
{
	...
	describe("make a bid", () =>
	{
		...
		it("should not make a bid if not enough tokens are available", async () =>
		{
			...
			let txBid2 = dex.connect(bob).acceptOffer(offerId, bat.address, amountBob)
		...
		it("should make a bid for 100% tokens", async () =>
		{
			...
			let txBid = await dex.connect(bob).acceptOffer(offerEvent.offerId, bat.address, amountBob)
			...
			checkEventExists(logs, "OfferAccepted", {
			...
			checkEventExists(logs, "OfferClosed", {
				offerId: offerEvent.offerId,
		...
		it("should make a bid for 50% tokens", async () =>
		{
			...
			let txBid = await dex.connect(bob).acceptOffer(offerEvent.offerId, bat.address, halfAmountBob)
			let receipt = await txBid.wait()
			let logs = parseLogs(receipt.logs)
